...
void allPrimes(ll a, ll b, vector<ll> & primes) {
	bool isPrime[b - a + 1];
	for (int i = 0; i <= b - a; i++) {
		isPrime[i] = true;
	}
	for (ll i = 0; ((primes[i]) * (primes[i])) <= b; i++) {
		int currPrimes = primes[i];

		// Just smaller or equal value to A
		ll base = (a / (currPrimes)) * (currPrimes);
		if (base < a) {
			base = base + currPrimes;
		}
		// Marking all multiples within A To B Range
		for (ll j = base; j <= b; j += currPrimes) {
			isPrime[j - a] = false;
		}
		// Special Case-> if base is currPrimes
		if (base = currPrimes) {
			isPrime[base - a] = true;
		}
	}
