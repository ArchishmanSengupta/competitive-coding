...
void solve() {
	ll d, sumTime, minTime[30], maxTime[30];
	cin >> d >> sumTime;
	fo(i, d) {
		cin >> minTime[i] >> maxTime[i];
	}
	int minSum = accumulate(minTime, minTime + d, 0);
	int maxSum = accumulate(maxTime, maxTime + d, 0);
	debug(minSum) debug(maxSum)
	if (minSum <= sumTime and sumTime <= maxSum) {
		cout << "YES" << nline;
		fo(i, d) {
			int res = min(minTime[i] + sumTime - minSum, maxTime[i]);
			debug(res)
			if (i + 1 < d) {
				cout << res << " ";
			}
			else cout << res << nline;
			sumTime = sumTime - (res - minTime[i]);
			debug(sumTime)
		}
	}
	else cout << "NO" << nline;
